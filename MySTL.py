...
def LineIntersection(l1, l2):
    ...
    def Rect(line):
        return Rectangle(min(line.p1.x, line.p2.x), max(line.p1.x, line.p2.x), min(line.p1.y, line.p2.y),
                         max(line.p1.y, line.p2.y))
...
def LineIntersectionPoint(l1, l2):
    ...
    tmpRight = (l1.p1.y - l2.p1.y) * (l1.p2.x - l1.p1.x) * (l2.p2.x - l2.p1.x) + l2.p1.x * (l2.p2.y - l2.p1.y) * (
                l1.p2.x - l1.p1.x) - l1.p1.x * (l1.p2.y - l1.p1.y) * (l2.p2.x - l2.p1.x)
    x = tmpRight / tmpLeft
    tmpLeft = (l1.p1.x - l1.p2.x) * (l2.p2.y - l2.p1.y) - (l1.p2.y - l1.p1.y) * (l2.p1.x - l2.p2.x)
    tmpRight = l1.p2.y * (l1.p1.x - l1.p2.x) * (l2.p2.y - l2.p1.y) + (l2.p2.x - l1.p2.x) * (l2.p2.y - l2.p1.y) * (
                l1.p1.y - l1.p2.y) - l2.p2.y * (l2.p1.x - l2.p2.x) * (l1.p2.y - l1.p1.y)
    y = tmpRight / tmpLeft;
    return Point(x, y)
...
def FutureGrenadePos(grenade, walls, future):
    ...
    for wall in walls:
        ...
        for li in (l1, l2, l3, l4):
            ...
            if re is not None and re is not (None, None):
...
def FutureBulletPos(bullet, walls, future):
    ...
    for wall in walls:
        ...
        if LineIntersection(l1, line) or LineIntersection(l2, line) or LineIntersection(l3, line) or LineIntersection(
                l4, line):
            return None
        if DisLinePoint(l1, pos2) < r or DisLinePoint(l2, pos2) < r or DisLinePoint(l3, pos2) < r or DisLinePoint(l4,
                                                                                                                  pos2) < r:
            return None
    return pos2
...
def HumanCanGotoPos(human, walls, pos):
    ...
    for wall in walls:
        ...
        if LineIntersection(l1, line) or LineIntersection(l2, line) or LineIntersection(l3, line) or LineIntersection(
                l4, line):
            return False
        if DisLinePoint(l1, pos) < r or DisLinePoint(l2, pos) < r or DisLinePoint(l3, pos) < r or DisLinePoint(l4,
                                                                                                               pos) < r:
            return False
    return True
...
if __name__ == '__main__':
    ...
    def drawRect(rect):
        pygame.draw.rect(screen, red, pygame.Rect(int(rect.left), int(rect.bottom), int(rect.right - rect.left),
